 ...
 *   导入 Excel/CSV —— 上传文件，工作簿里每张表都读
 *   粘贴名单     —— 从花名册直接粘文本
 * 两条路认**同样的四列**「学号 姓名 班级 性别」、同样的规则、同样的报错措辞
 * （后端共用 services/roster.py），换个入口不用重新适应一遍。
 * 初始密码都是学号。
 */
...
<template>
  ...
  <el-dialog v-model="importDlg" title="从 Excel / CSV 导入学生" width="540px">
    ...
    <div class="tip">
      ...
      <b>工作簿里每一张表都会读</b> —— 一个年级一张表、按班分表都行，
      报错会指明是哪张表的第几行。模板自带的「填写说明」「可用班级」两页会自动跳过。<br />
  ...
  <el-dialog v-model="batchDlg" title="粘贴名单批量建号" width="560px">
    ...
    <div class="tip">
      一行一个学生，四列：<b>学号、姓名、班级、性别</b>，后两列选填 ——
      和「导入 Excel/CSV」认的是同一套。<br />
      <b>从 Excel 直接复制粘贴即可</b>，那样自带制表符分隔，四列都能认出来。
      手打的话「学号 姓名」中间用空格隔开就行。<br />
      <b>单个空格不当列分隔</b>，所以姓名里带空格（如「钱 七」）不会被拆散；
      要粘多列请用 Excel 复制、或者用逗号隔开。<br />
      <b>班级列留空</b>的行归到上面选的班。已存在的学号自动跳过，
