...
> - 每个文件是干什么的、改功能该动哪里 → [docs/项目结构.md](docs/项目结构.md)
> - 加了新功能怎么发到服务器 → [docs/开发与发布流程.md](docs/开发与发布流程.md)
> - 部署到公网服务器 → [docs/部署指南.md](docs/部署指南.md)
> 要部署到有公网 IP 的机器给全校用 → 看 [docs/部署指南.md](docs/部署指南.md)
> 
...
├─ config.yaml               集中配置：学校名、默认题量、及格线、导入规则……
...
系统"怎么用"的部分集中在根目录的 `config.yaml`，改完重启后端生效
（双击 `一键停止.bat` 再双击 `一键启动.bat`；Docker 部署用 `docker compose restart backend`）。

| 段落 | 管什么 |
|---|---|
| `school` | 印在卷头的学校名 |
| `paper` | 默认卷名、考试时长、各题型默认数量、是否打乱选项、大题序号、卷号前缀 |
| `exam` | 及格线、学生链接口令长度、发布考试四个开关的默认状态 |
| `bank` | 首次建库用的知识范围与题型；是否每次启动都以本文件为准同步 |
| `import` | 导入表头、题型识别关键词、判断题答案的各种写法、选项分隔符、最多几个选项 |
| `upload` | 上传大小上限、允许的图片格式 |

**三条要点：**

1. **密钥不在这里**。数据库密码、JWT 密钥、管理员初始密码在 `backend/.env`，
   因为 `config.yaml` 要提交进 Git，写进来等于公开。
2. **知识范围的日常增删走界面**，不要改这里。`bank.scopes` 只在第一次建库时用作初始值，
   之后以数据库为准（想让文件说了算，把 `bank.sync_on_start` 改成 `true`）。
3. **改错不会静默失效**。键名拼错、类型不对、取值超范围，后端启动时直接报错并指出是哪一项：

   ```
   配置文件有问题，系统没有启动：
     D:\...\config.yaml
     下面这些项填得不对：
       exam → pass_score：Input should be less than or equal to 100
   ```

前端也读同一份配置（启动时调 `GET /api/config`），所以组卷页的默认题量、
成绩表的及格线标红，改 YAML 之后一起变。
